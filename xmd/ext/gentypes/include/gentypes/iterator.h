...
template <typename T, typename Idx> class iterator {
public:
  inline explicit iterator(ptr<T> data, Idx idx) : data_{data}, idx_{idx} {};

  ptr<T> data() const { return data_; }
  ...
  iterator operator++(int) const { return iterator(data_, idx_++); }
  ...
  ref<T> operator[](Idx idx) const { return data_[idx]; }

  ref<T> operator*() const { return data_[idx_]; }
